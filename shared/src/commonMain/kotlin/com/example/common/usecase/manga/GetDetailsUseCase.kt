...
class GetDetailsUseCase(private val animeRepository: AnimeRepository){
    operator fun invoke(contentType: String?, id: String): Flow<StateListWrapper<ContentDetail>> {
        return flow {
            emit(StateListWrapper.loading())
            println("ID = $id")
            val res = animeRepository.getAnimeDetails(id)
            println("ID = ${res.message}")
            println("ID = ${res.data?.data}")
            val state = when (res) {
